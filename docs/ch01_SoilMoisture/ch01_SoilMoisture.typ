...
#h(2em)
BEPS 采用一维土壤水模型，土壤默认划分为5层，每层厚度为$Delta z = [0.05, 0.10, 0.20, 0.40, 1.25] "m"$，土壤水运动模块的主要变量见图#[@fig_SM]与表#[@table_module_SM]。
...
#h(2em)
设$beta$为根系分布衰减系数，$Z_i$为从地表到第$i$层底部的累计深度（cm），根系分布比例$f_("root",i)$按指数衰减计算：
...
其中，$psi_("min")$为水分胁迫开始增强的吸力水头阈值，$alpha$控制胁迫随吸力水头增加而增强的速率。

最后，再根据根系分布进行加权，得到未归一化吸水权重：

$ a_i = f_("root",i) f_("stress",i) $

$ w_i = a_i / sum_(j=1)^N a_j $ <eq_wi>

将式#[@eq_wi]代回式#[@eq_S1]和#[@eq_Si]，即可得到每层土壤的蒸散汇项$S_i$。
