...
== 3.1 Monin-Obukhov 相似理论（MOST）风速廓线
...
#h(2em)
式@eq-ustar 存在循环依赖：计算 $u_star$ 需要 $L_MO$，而 $L_MO$ 又依赖 $u_star$：
...
=== 3.4.3 分层阻力的精确计算（$gamma$ 相同场景）

==== 3.4.3.1 林下植被到冠层顶部的阻力 ($r_(a,u)$)
...
=== 3.4.4 ModernBEPS 模型中的实现细节（分层 $gamma$ 场景）
...
#box-blue[
  *有效风速的物理意义* \
  模型中计算衰减到零平面位移高度 $d$ 处的风速 $u_o$，是为了提取出一个*“最能代表广大叶片实际感受到的平均微风速度”*。用这个有效风速去算出来的努塞尔数（Nusselt number）和叶片边界层阻力 $r_b$，才最符合整棵树的宏观统计规律。
]

==== 3.4.4.2 林冠层空气动力学阻力 ($r_(a,u)$)
积分区间从林下植被顶部 $h_u$ 到大树顶部 $h$。此空间内的湍流受主林冠的热量消光系数 $gamma_(o,h)$ 控制：
$ r_(a,u) = h / (gamma_(o,h) K_(h,o)) [exp(gamma_(o,h) (1 - h_u/h)) - 1] $

==== 3.4.4.3 底层空间空气动力学阻力 ($r_(a,g)$)
...
#h(2em)
模型中`ra_g` 是*地表至参考高度的总串联阻力*（即 $r_(a,g,"local") + r_(a,u) + r_(a,o)$），专用于 `evaporation_soil` 模块计算地表蒸发时的 $G_("heat,g") = 1 / r_(a,g)$。需要注意，这与表征冠层内部阻力分配的局部 $r_(a,g,"local")$ 概念不同。
