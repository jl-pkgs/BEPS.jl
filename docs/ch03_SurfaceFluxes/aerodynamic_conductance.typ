...
== 1.1 Monin-Obukhov 相似理论（MOST）风速廓线
...
=== 1.4.3 分层阻力的精确计算（$gamma$ 相同场景）

==== 1.4.3.1 林下植被到冠层顶部的阻力 ($r_(a,u)$)
...
=== 1.4.4 ModernBEPS 模型中的实现细节（分层 $gamma$ 场景）
...
#h(2em)
模型中`ra_g` 是*地表至参考高度的总串联阻力*（即 $r_(a,g,"local") + r_(a,u) + r_(a,o)$），专用于 `evaporation_soil` 模块计算地表蒸发时的 $G_("heat,g") = 1 / r_(a,g)$。这与表征冠层内部阻力分配的局部 $r_(a,g,"local")$ 概念不同，两者不可混用。
