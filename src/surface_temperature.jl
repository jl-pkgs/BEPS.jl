...
function surface_temperature_jl(T_air::FT, rh_air::FT, z_snow::FT, z_water::FT,
  ...
  # 雪热导率
  κ_dry_snow::FT = 0.021 + 4.2e-4 * ρ_snow + 2.2e-9 * ρ_snow^3
  ...
  if z_snow <= 0.02
    # Case 1: 无雪或极浅雪 (≤2cm)
    ΔE = cp_soil1 * 0.02 / Δt
    T_ground = (T_ground_last * ΔE * ra_g * z_soil1 + Gg * ra_g * z_soil1 +
                ρₐ * cp * T_air * z_soil1 + ra_g * λ_soil1 * T_soil1_last) /
  ...
  elseif z_snow > 0.02 && z_snow <= 0.05
    ...
    T_soil0 = (T_soil0_last * ΔE_soil * ra_g * z_soil1 + Gg * ra_g * z_soil1 +
               ρₐ * cp * T_air * z_soil1 + 2 * ra_g * λ_soil1 * T_soil1_last) /
    ...
    T_snow = (T_snow_last * ΔE_snow * ra_g * z_snow + Gg * ra_g * z_snow +
              ρₐ * cp * tempL_u * z_snow + ra_g * κ_dry_snow * T_soil_surf_last) /
    ...
    T_snow = (T_snow_last * ΔE_snow * ra_g * 0.04 + Gg * ra_g * 0.02 +
              ρₐ * cp * T_air * 0.04 + ra_g * κ_dry_snow * T_snow1_last) /
