...
# 新版本：JAX 风格 (st, ps) 签名
function UpdateHeatFlux(st::SoilState, ps::BEPSmodel, Tair_annual_mean::Float64, period_in_seconds::Float64)
  (; G, Tsoil_c, Tsoil_p, κ) = st
  (; dz) = ps
  n = st.n_layer
  ...
  S = 0.0
  for i in 1:n
    Tsoil_c[i] = Tsoil_p[i] + (G[i] - G[i+1] + S) / (st.Cs[i] * dz[i]) * period_in_seconds
    Tsoil_c[i] = clamp(Tsoil_c[i], -50.0, 50.0)
  end
  Update_ice_ratio(st, ps)
  Tsoil_p[1:n] .= Tsoil_c[1:n]
end
...
# 新版本：JAX 风格 (st, ps) 签名
function Update_Cs(st::SoilState, ps::BEPSmodel)
  (; θ, ice_ratio) = st
  (; ρ_soil, V_SOM) = ps.thermal
  
  for i in 1:st.n_layer
    term1 = 2.0 * 1.0e+6 * ρ_soil[i] / 2650.0 # soil solid, like Quartz
    term2 = 1.0e+6 * θ[i] * (4.2 * (1 - ice_ratio[i]) + 2.09 * ice_ratio[i]) # water and ice
    term3 = 2.5 * 1.0e+6 * V_SOM[i] # soil organic matter
    st.Cs[i] = term1 + term2 + term3
  end
end
...
# 新版本：JAX 风格 (st, ps) 签名
function Update_ice_ratio(st::SoilState, ps::BEPSmodel)
  (; θ, θ_prev, Tsoil_c, Tsoil_p, ice_ratio) = st
  (; dz) = ps
  Lf0 = 3.34 * 100000  # latent heat of fusion (liquid: solid) at 0C
  
  @inbounds for i in 1:st.n_layer
    if Tsoil_p[i] >= 0.0 && Tsoil_c[i] < 0.0 && ice_ratio[i] < 1.0
      Gsf = (0.0 - Tsoil_c[i]) * st.Cs[i] * dz[i]
      ice_ratio[i] += Gsf / Lf0 / 1000.0 / (θ[i] * dz[i])
      ice_ratio[i] = min(1.0, ice_ratio[i])
      Tsoil_c[i] = 0
    elseif Tsoil_p[i] <= 0.0 && Tsoil_c[i] > 0.0 && ice_ratio[i] > 0.0
      Gsm = (Tsoil_c[i] - 0.0) * st.Cs[i] * dz[i]
      ice_ratio[i] -= Gsm / Lf0 / 1000.0 / (θ[i] * dz[i])
      ice_ratio[i] = max(0.0, ice_ratio[i])
      Tsoil_c[i] = 0
    end
...
# 新版本：JAX 风格 (st, ps) 签名
@fastmath function UpdateSoilThermalConductivity(st::SoilState, ps::BEPSmodel)
  (; θ, ice_ratio, κ) = st
  (; θ_sat) = ps.hydraulic
  (; κ_dry) = ps.thermal
  ...
  @inbounds for i in 1:st.n_layer
    ...
    κ[i] = (k_dry_i * tmp2 * tmp3 - 0.15) * tmp4 + 0.15
    κ[i] = max(κ[i], 0.15)
  end
end
